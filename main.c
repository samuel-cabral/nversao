...
void send_async(int *value, int index) {
    canal[index] = *value;
    return;
}

int get_majority_vote(int votos[], int *versao_erro) {
...
void *driver(void *threadno) {
    ...
    majority = get_majority_vote(votos, &error_thread);

    printf("\nVoto majoritario: %d", majority);

    if(error_thread == 0) {
        send_async(&ok_code, REC_A);
        send_async(&ok_code, REC_B);
        send_async(&ok_code, REC_C);
    }

    else if(error_thread == 1) {
        send_async(&error_code, REC_A);
        send_async(&ok_code, REC_B);
        send_async(&ok_code, REC_C);
    }
